...
**读取配置与写入配置同样属于特权操作。**`settings.describe` 与 `credentials.describe` 遵循已声明的 `trustedHosts` 策略，因此回环和明确受信任的 LAN 页面可以使用完整配置面，未受信任的 authority 仍在其外。模型目录（`llm.providers`、`llm.models`）携带提供方 id、显示名与模型列表，同样可供受信任 LAN 页面使用。宿主原生操作、设置文档打开和 preset 创作仍只限回环。这是可达性授权而非认证；需要用户认证的部署必须在 Web 前放置经过认证的代理。这条边界由一台真实 HTTP 服务器来断言，而不是手工拼装的请求，因为真正决定它的，是浏览器实际发出的那个 `Host` 头。
...
`trustedHosts` 部署下的 LAN 客户端可以像回环页面一样渲染和管理设置页，包括脱敏读取与凭据写入；未受信任的 authority 仍在配置面之外。settings seam 暴露的已注册 namespace 可以在 Web 上配置，提供方目录继续拥有产品的模型提供方名单。宿主原生操作与 preset 创作仍保留仅限回环的限制。`SettingsDescriptor` 新增了必填的 `revision`，因此以编程方式构造 descriptor 形状值的地方都必须提供它；`settings/document-updated` 是一个新事件，提供方侧的任何 listener 现在都可以观察它。忽略 `expectedRevision` 的客户端，其后写胜出的语义完全不变。延后事项：fail-closed 的协议 describe（连同它所承载的 `headers` 与信封净化工作），以及一套不含可执行代码的浏览器 schema 协议。
