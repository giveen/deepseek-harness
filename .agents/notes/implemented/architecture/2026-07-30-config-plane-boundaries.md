...
**Reading configuration is as privileged as writing it.** `settings.describe` and `credentials.describe` follow the declared `trustedHosts` policy, so loopback and explicitly trusted LAN pages can use the complete configuration plane while untrusted authorities remain outside it. The model catalog (`llm.providers`, `llm.models`) carries provider ids, display names, and model lists, and is likewise available to trusted LAN pages. Host-native actions, settings-document opening, and preset authoring remain loopback-only. This is a reachability grant rather than authentication; deployments needing user authentication must put an authenticated proxy in front of Web. The boundary is asserted over a real HTTP server rather than a hand-assembled request, because the `Host` header a browser actually sends is what decides it.
...
A LAN client on a `trustedHosts` deployment can render and manage the same settings page as loopback, including redacted reads and credential writes; untrusted authorities remain outside the configuration plane. A plugin that registers a settings namespace is web-configurable when it is exposed by the settings seam, while the provider directory continues to own the product's model-provider roster. Host-native actions and preset authoring retain their loopback restriction. `SettingsDescriptor` gained a required `revision`, so any programmatic constructor of a descriptor-shaped value must supply it, and `settings/document-updated` is a new event any provider-side listener may now observe. Clients that ignore `expectedRevision` keep last-write-wins semantics unchanged. Deferred: the fail-closed wire describe (with the `headers` and envelope-sanitization work it carries), and a non-executable browser schema protocol.
