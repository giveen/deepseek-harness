...
**The welcome notice reuses the existing durable field.** Its exact copy and version live in `onboarding-copy.ts`. Loopback and declared trusted LAN clients compare and write `ui-onboarding.welcomeNoticeVersion` through the existing settings API, and only Continue acknowledges the current version. Untrusted remote clients receive the Host trust fence's 403 rather than a separate process-local settings store. No Host schema or persistence implementation changes.
