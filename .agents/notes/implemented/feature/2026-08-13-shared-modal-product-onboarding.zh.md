...
**欢迎声明复用既有持久化字段。** 完整文案与版本由 `onboarding-copy.ts` 持有。回环和已声明的受信任 LAN 客户端通过既有 settings API 比较和写入 `ui-onboarding.welcomeNoticeVersion`，且只有点击「继续」才确认当前版本。未受信任的远程客户端会收到 Host 信任栅栏的 403，而不是另一份进程内 settings 存储。不改变 Host schema 或持久化实现。
