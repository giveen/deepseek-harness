...
function runScript(script: string, environment: NodeJS.ProcessEnv): void {
  ...
  if (packageManager === undefined || packageManager === '') throw new Error('build: npm_execpath is unavailable; invoke the build through a package script')
  ...
  if (result.status !== 0) throw new Error(`build: ${script} exited with ${String(result.status ?? result.signal)}`)
}
...
function main(): void {
  ...
  const parentEnvironment = { ...process.env, DSH_CLIENT_COMMIT_HASH: repositoryCommitHash(root, process.env) }
  ...
  console.log(`build: recorded ${String(record.artifacts.fileCount)} client artifact(s) with ${String(Object.keys(record.environment).length)} public value(s)`)
}
