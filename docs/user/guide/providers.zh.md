...
因此自定义提供方下的视觉模型需要逐个声明。打开**自定义设置**，展开模型行并启用**支持图片输入**。该控件会为该模型写入 `input: [text, image]`；关闭时会明确写入 `input: [text]`。也可以直接在 `$DSH_HOME/settings.yaml` 中编辑同一字段：
...
- **图片在发送前被拒绝**：该模型未声明图片模态。在**设置 → 模型**中打开**自定义设置**，展开模型行并启用**支持图片输入**；这会写入 `input: [text, image]`。DeepSeek 自身的 chat-completions 路由是纯文本的，且无法通过配置改变。
