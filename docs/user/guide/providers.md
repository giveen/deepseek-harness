...
A vision model on a custom provider therefore needs one per-model declaration. Open **Customized settings**, expand the model row, and enable **Supports image input**. The control writes `input: [text, image]` for that model; leave it off to write `input: [text]` explicitly. You can also edit the same field directly in `$DSH_HOME/settings.yaml`:
...
- **An image is refused before sending** — The model declares no image modality. In **Settings → Models**, open **Customized settings**, expand the model row, and enable **Supports image input**; this writes `input: [text, image]`. DeepSeek's own chat-completions route is text-only and cannot be configured otherwise.
