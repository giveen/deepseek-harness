...
- **Routed policy** — proactive pressure resolves capacity from the adapter that owns the latest durable provider/model route, then scales the default policy plus an optional exact-target override into concrete token budgets. OpenAI-compatible adapters may refresh that capacity from their local `/models` metadata using the configured TTL; model discovery remains advisory and is not consulted by compaction itself.
