...
| `browser_snapshot` | 获取最新的可访问性树、页面专属元素引用和仅供 UI 使用的视口截图。 |
...
启用 `autoStart: true` 时，如果内置服务无法启动，或在 30 秒内没有对 `/health` 作出响应，插件激活会失败。插件负责管理该进程，并在卸载时等待它停止。插件会在把结果返回给模型前验证 camofox JSON 响应；格式错误的响应和 HTTP 错误都会被拒绝。每次浏览器操作成功后，插件都会尽力捕获视口 PNG，将保留的字节限制为 1.5 MB，并把截图放入可重放的展示元数据；模型可见的结果文本仍只包含可访问性内容或操作文本。它使用调用 agent 的会话 id 作为 `userId`，并以 `sessionKey: dsh-browser` 将该 agent 的标签页分组；不同 agent 不共享浏览器 cookie。当前客户端面向回环地址上的 camofox 服务；由于插件尚未携带访问密钥凭据，不要把服务暴露到回环地址之外。二进制刷新失败不会使安装失败，但启动时仍会通过受管服务的就绪检查报告缺失或不可用的浏览器。
...
模型会收到五个工具：`browser_navigate`、`browser_snapshot`、`browser_click`、`browser_type` 和 `browser_scroll`。导航结果提供 `tabId`；快照提供页面专属的 `e5` 等引用；点击接受一个当前引用或 CSS 选择器；输入接受当前引用；滚动返回方向和距离。提示会要求模型在使用引用前，以及导航、改变页面的操作或滚动后重新获取快照。Web 客户端会在对应的工具行中打开 Browser 面板并显示最近一次成功捕获的视口；截图属于 UI 元数据，不会加入模型提示。
...
- 受管服务仅监听回环地址且没有访问密钥认证；在加入请求认证前，不支持把它暴露到 `127.0.0.1` 之外。
- 浏览器状态按 agent session 隔离，不会有意在 agent 之间共享。
- 截图捕获是尽力而为的；缺少截图或 PNG 超过限制时，浏览器操作及其文字结果仍然可用，但不会显示视觉面板。
