...
The package also fills `conversation.details.tool` with `ToolDetails`. The row and details renderers share the same pure card models for `terminal`, `read`, `diff`, `search`, `web`, and `browser` render intents. Camofox browser results open a bounded screenshot panel in the chat row and reuse that panel in details. Unknown intent tags and malformed wire card data fall back to flattened Tool result text.
...
This package currently owns the generic fallback and the built-in shell/pwsh, read, write/edit, grep/glob, web, camofox browser, todo, question, and Code Dispatch presentations. `ui-skill` demonstrates a business-owned registration for `skill`.
...
- Browser screenshots are bounded host metadata; the row cannot show a live page or recover a screenshot that camofox did not capture.
