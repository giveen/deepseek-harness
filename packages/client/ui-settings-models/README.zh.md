...
声明步骤在 `src/onboarding-copy.ts` 中持有完整文案和版本。回环和受信任 LAN 页面通过同一个 Host settings API 比较并写入 `ui-onboarding.welcomeNoticeVersion`；只有明确点击「继续」才会记录当前版本。Host 未信任的页面会收到 API 的 403，而不会静默创建另一份设置存储。
