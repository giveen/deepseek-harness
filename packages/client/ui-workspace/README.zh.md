...
对话 view ring 还会在 Chat 和 Trajectory 旁贡献 **文件** 与 **提交** 两个标签。文件标签为当前 Session 的 Workspace 加载受 Host 限制的递归文件树；点击文件会通过现有的原生打开器交给 Host 处理。提交标签按最新优先加载分页 Git 历史，点击每条提交即可在原位展开完整消息，并使用返回的提交游标请求更早页面。Host 会排除 `.git` 与 `node_modules`、限制遍历范围；如果没有 Git 历史，界面只显示视图内错误，不会导致对话失败。
