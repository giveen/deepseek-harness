...
- **未受信任的远程浏览器没有持久化设置**：Host 信任栅栏会在回环和已声明的受信任 authority 之外返回 403；受信任 LAN 页面与回环页面使用同一个持久化 scope。
