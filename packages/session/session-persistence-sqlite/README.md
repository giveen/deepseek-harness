...
An opt-in SQLite `SessionPersistence` provider — a second provider ([session persistence](../../../.agents/notes/implemented/architecture/2026-06-14-session-persistence.md)) satisfying the same contract as `dsh-session-persistence-jsonl` while using schema-17 physical rows. Compatible assistant chunk runs are packed, large payloads are selectively Zstandard-compressed, and provenance arrays use delta-encoded varints; all reads restore the exact logical `SessionEvent[]`.
...
Schema 17 keeps an `events(session_id, seq)` physical index. Scalar rows store one logical event. Consecutive same-block text, reasoning, and tool-call deltas use the physical tags `text-chunks`, `reasoning-chunks`, and `tool-call-chunks`; a packed row stores its first sequence/time and shared payload, and represents at most 1,024 events and 1 MiB of uncompressed UTF-8 data. Packed rows use `ignorable=0` as a discriminator and have null surface fields; scalar rows use `ignorable=1` only for logical ignorable events, so a future ignorable event may safely reuse a storage-tag name. Unknown fields, surface metadata, gaps, incompatible chunk identity, and unsafe timestamps remain scalar. The nullable `surface_op` field stores optional surface metadata (see [session surface](../../../.agents/notes/implemented/architecture/2026-06-18-session-surface.md)); `source_event_seqs` is stored as a complete delta-encoded BLOB. Reads expand physical rows before returning events. Out-of-log metadata (`SessionHeader`), a per-materialization incarnation id, and a monotonic per-log revision live in a `sessions` row; `createdAt` is a non-negative safe integer stored in a strict `INTEGER` column. A singleton state row carries the immutable store id. A `sessions` row is written only by the first `append` — its existence is the lazy-materialization signal (`list` reports exactly the sessions that have a row). Deletion marks live in `session_deletion_marks`; they are durable maintenance state, not session events, and are removed with their session by the foreign-key cascade.
...
Like the JSONL backend, the plugin copies each frozen `session/event` into one controller per live session. The first pending event starts the configured fixed batching window, and later events join without resetting it. Each durable batch runs `BEGIN IMMEDIATE`, validates the physical tail, packs only newly appended compatible events, inserts the records, and increments the revision once; it never rewrites earlier rows. `session/flush` cancels the wait and drains current and pending batches. The controller persists a fork's seed once, keeps a write cursor so resume never re-appends stored events, and seeds live sessions on apply because HMR does not replay `session/created`. Dispose drains every retained controller before closing the database.
...
- **Only a pristine new database or the current owned schema-17 `SCHEMA_VERSION` opens** — unversioned schema objects, foreign application identities, and every other schema version are rejected rather than migrated (unreleased software; no persisted user data to preserve).
