/**
 * Schema-owned SQLite physical-row compression and decoding.
...
import { decodeSerializedChunkRow, type ChunkRow, MAX_PACKED_DATA_BYTES, type StorageRecord } from './codec.ts'
import type { EventRow } from './schema.ts'
/** One physical row ready for SQLite binding. */
...
/** Values smaller than this remain SQLite text. */
...
function isChunkTag(value: string): value is ChunkTag { return (CHUNK_TAGS as readonly string[]).includes(value) }
/**
 * Decode one physical SQLite row into all represented logical events.
...
/**
 * Convert one logical or packed record to SQLite column values.
 * @param record - scalar event or packed chunk record.
 * @returns values for one physical SQLite insert.
 */
export function bindRecord(record: StorageRecord): BoundRecord {
  if (isChunkRow(record)) {
    return {
      seq: record.seq0, type: record.type, time: record.time0,
      data: encodeData(JSON.stringify(record.data)), sourceEventSeqs: null, surfaceOp: null, ignorable: PACKED_ROW_SENTINEL,
    }
  }
  const event = record
  ...
  return {
    ...
    sourceEventSeqs: surface.sourceEventSeqs === undefined ? null : encodeSourceEventSeqs(surface.sourceEventSeqs),
...
function decodeData(value: string | Uint8Array, maxOutputLength?: number): string {
  ...
  const decoded = maxOutputLength === undefined ? zstdDecompressSync(value) : zstdDecompressSync(value, { maxOutputLength })
...
function encodeSourceEventSeqs(values: readonly number[]): Uint8Array {
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    const encoded = index === 0 ? value : value >= previous ? (value - previous) * 2n : ((previous - value) * 2n) - 1n
...
function appendVarint(bytes: number[], value: bigint): void {
  let remaining = value
  while (remaining >= 0x80n) { bytes.push(Number(remaining & 0x7fn) | 0x80); remaining >>= 7n }
...
function decodeSourceEventSeqs(bytes: Uint8Array): number[] {
  ...
  while (offset < bytes.length) {
    ...
    values.push(Number(value)); previous = value
  }
  return values
}
function readVarint(bytes: Uint8Array, offset: number, limit: bigint): { readonly value: bigint; readonly offset: number } {
  let value = 0n
  let shift = 0n
  while (offset < bytes.length) {
    ...
    if ((byte & 0x80) === 0) {
      if (shift > 0n && (byte & 0x7f) === 0) throw new Error('malformed source_event_seqs storage value: non-canonical varint')
...
function isChunkRow(record: StorageRecord): record is ChunkRow { return isChunkTag(record.type) && 'seq0' in record && !('seq' in record) }
function decodeScalarRow(row: EventRow): SessionEvent {
  const surfaceFields = {
    ...row.source_event_seqs === null ? {} : { sourceEventSeqs: decodeSourceEventSeqs(row.source_event_seqs as Uint8Array) },
    ...row.surface_op === null ? {} : { surfaceOp: JSON.parse(row.surface_op) as SessionEvent<SurfaceEventType>['surfaceOp'] },
  }
  return {
    type: row.type as SessionEvent['type'], seq: row.seq, time: row.time,
    data: JSON.parse(decodeData(row.data)) as SessionEvent['data'],
    ...surfaceFields, ...row.ignorable === 1 ? { ignorable: true as const } : {},
  } as SessionEvent
}
/**
 * Scan physical rows into a logical contiguous prefix and optional tail marker.
 ...
 * @returns the preserved logical prefix and optional physical tail marker.
 */
export function scanRows(rows: readonly EventRow[], base = 0): { preserved: SessionEvent[]; tornFrom?: number } {
  ...
  for (let index = rows.length - 1; index >= 0; index -= 1) {
    try {
      if (decodeRow(rows[index] as EventRow).some(event => event.type === 'turn/end')) { lastTurnEndRow = index; break }
    } catch { /* malformed physical rows cannot prove a committed boundary */ }
  }
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
    ...
    try { logicalEvents = decodeRow(physical) } catch { /* committed-prefix rule below decides whether this is fatal */ }
    ...
    for (const event of logicalEvents) {
      if (event.seq !== expected) { contiguous = false; break }
      expected += 1
    }
    if (!contiguous) {
      if (rowIndex <= lastTurnEndRow) throw new Error(`corrupt session log: invalid committed physical row at seq ${physical.seq}`)
