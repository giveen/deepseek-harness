/**
 * Schema-17 physical chunk-row codec. The package owns the durable tags,
 * validation, and physical row-size limits independently from other formats.
...
interface RunDataBase { readonly turn: number; readonly step: number; readonly index: number; readonly dt: number[] }
interface TextRunData extends RunDataBase { readonly texts: string[] }
...
/** Maximum UTF-8 bytes in one packed physical record data value. */
...
function classify(event: SessionEvent): DeltaKind | undefined {
  ...
  switch (chunk.type) {
    ...
    case 'reasoning-delta':
      return hasExactKeys(chunk, ['type', 'index', 'text']) && typeof chunk.text === 'string' ? chunk.type : undefined
    case 'tool-call-delta': {
      ...
      return validKeys && typeof chunk.id === 'string' && typeof chunk.argumentsDelta === 'string' ? chunk.type : undefined
    }
...
function indexOf(event: DeltaEvent): number { return (event.data.chunk as { readonly index: number }).index }
...
function buildRow(kind: DeltaKind, run: readonly DeltaEvent[]): ChunkRow {
  ...
  if (kind === 'tool-call-delta') {
    ...
    return {
      type: 'tool-call-chunks', ...envelope,
      data: {
        ...base,
  ...
  const data = { ...base, texts: run.map(event => (event.data.chunk as { readonly text: string }).text) }
  return kind === 'text-delta' ? { type: 'text-chunks', ...envelope, data } : { type: 'reasoning-chunks', ...envelope, data }
}
function packedDataBytes(row: ChunkRow): number { return Buffer.byteLength(JSON.stringify(row.data)) }
function malformed(tag: string, reason: string): never { throw new Error(`malformed ${tag} storage row: ${reason}`) }
function emitBoundedRun(out: StorageRecord[], kind: DeltaKind, completeRun: readonly DeltaEvent[]): void {
  ...
  while (completeRun.length - offset >= MIN_PACKED_ROW_MEMBERS) {
    ...
    if (packedDataBytes(largest) <= MAX_PACKED_DATA_BYTES) { out.push(largest); offset += high; continue }
    high -= 1
    let accepted = 0
    ...
    while (low <= high) {
      ...
      if (packedDataBytes(candidate) <= MAX_PACKED_DATA_BYTES) { accepted = middle; acceptedRow = candidate; low = middle + 1 }
      else high = middle - 1
    }
    if (accepted === 0) { out.push(completeRun[offset] as DeltaEvent); offset += 1; continue }
...
export function packChunkRuns(events: readonly SessionEvent[]): StorageRecord[] {
  ...
  let run: DeltaEvent[] = []
  const flush = (): void => { if (kind === undefined) out.push(...run); else emitBoundedRun(out, kind, run); kind = undefined; run = [] }
  for (const event of events) {
    const nextKind = classify(event)
    if (nextKind === undefined) { flush(); out.push(event); continue }
    const delta = event as DeltaEvent
    const previous = run.at(-1)
    if (nextKind === kind && previous !== undefined && continues(previous, delta, nextKind)) { run.push(delta); continue }
    flush(); kind = nextKind; run = [delta]
  }
  flush()
  return out
}
function validateRunData(tag: string, data: Record<string, unknown>, payloadKey: 'texts' | 'args', serializedBytes?: number): string[] {
  ...
  if (!Array.isArray(payload) || payload.length < MIN_PACKED_ROW_MEMBERS || payload.length > MAX_PACKED_ROW_MEMBERS || payload.some(member => typeof member !== 'string')) {
  ...
  if ((serializedBytes ?? Buffer.byteLength(JSON.stringify(data))) > MAX_PACKED_DATA_BYTES) malformed(tag, `data exceeds ${MAX_PACKED_DATA_BYTES} UTF-8 bytes`)
  return payload as string[]
}
function validateRow(value: Record<string, unknown>, tag: ChunkRow['type'], serializedBytes?: number): ChunkRow {
  ...
  if (tag === 'tool-call-chunks') {
    ...
    if (!withName && !hasExactKeys(data, ['turn', 'step', 'index', 'id', 'dt', 'args'])) malformed(tag, 'invalid tool-call data fields')
  ...
  for (const gap of data.dt as number[]) { time += gap; if (!Number.isSafeInteger(time)) malformed(tag, 'member times exceed safe integers') }
...
function expandRow(row: ChunkRow): SessionEvent[] {
  ...
  for (let index = 0; index < members.length; index += 1) {
    ...
    switch (row.type) {
      case 'text-chunks': chunk = { type: 'text-delta', index: row.data.index, text: members[index] as string }; break
      case 'reasoning-chunks': chunk = { type: 'reasoning-delta', index: row.data.index, text: members[index] as string }; break
      case 'tool-call-chunks': chunk = { type: 'tool-call-delta', index: row.data.index, id: row.data.id, ...Object.hasOwn(row.data, 'name') ? { name: row.data.name as string } : {}, argumentsDelta: members[index] as string }; break
    }
    events.push({ type: 'assistant/chunk', seq: row.seq0 + index, time, data: { turn: row.data.turn, step: row.data.step, chunk } })
  }
  return events
}
/**
 * Decode one scalar or packed schema-17 record.
 * @param value - parsed physical record.
 * @returns every logical event represented by the record.
 */
export function decodeStorageRecord(value: unknown): SessionEvent[] {
  if (!isRecord(value)) return [value as SessionEvent]
  const tag = value.type
  if (tag !== 'text-chunks' && tag !== 'reasoning-chunks' && tag !== 'tool-call-chunks') return [value as SessionEvent]
  return expandRow(validateRow(value, tag))
}
/**
 * Decode one packed row from its exact uncompressed data value.
 * @param tag - packed physical tag.
 * @param seq0 - first represented logical sequence number.
 * @param time0 - first represented logical timestamp.
 * @param serializedData - uncompressed JSON data value.
 * @returns every logical event represented by the row.
 */
export function decodeSerializedChunkRow(tag: ChunkRow['type'], seq0: number, time0: number, serializedData: string): SessionEvent[] {
