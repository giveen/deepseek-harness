...
describe('settings domain', () => {
  ...
  it('serves every registered namespace, including one this repository never named', async () => {
    ...
    // The plane follows the connection Host-trust policy and stays
    // secret-redacted; which surface renders a namespace is the browser's
    // decision, not this proxy's.
    const ctx = await harness()
