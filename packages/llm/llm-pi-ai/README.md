  ...
  config:
    providers:
      ...
      openai:
        ...
        contextMetadataRefreshMs: 300000
        reasoning: high
        retryPolicy:
          mode: normal
          maxRetries: 3
...
For OpenAI-compatible routes with an explicitly configured `baseURL`, an omitted `contextWindow` is refreshed from `GET /models` when the exact model is resolved and again after `contextMetadataRefreshMs` (five minutes by default). The adapter reads the standard `context_window`/`context_length` fields and common local-server names including `max_model_len`, `max_model_length`, `num_ctx`, `n_ctx`, `context_size`, `max_seq_len`, and `max_sequence_length`. Explicit model configuration wins; otherwise a successful server value wins over the catalog or route fallback. A failed or incomplete probe keeps the last successful value, then the catalog or configured fallback, so metadata availability cannot break a model request. Set the interval to `0` to probe every exact-model resolution.
...
Most listings disclose an id and nothing else; `context_window`/`context_length` and `max_output_tokens`/`max_tokens` are read when a gateway supplies them, and common local aliases such as `max_model_len` and `num_ctx` are accepted for context capacity. Entries without a usable id are skipped rather than failing the whole listing, and everything else the adopting surface still owes. The reply is read under a four-megabyte ceiling enforced on the bytes actually received — the endpoint is a URL the user typed, so a declared length is checked first but never trusted as the bound. An unreachable endpoint, a refused credential, a non-JSON body, and a body with no `data` array all fail with `DISCOVERY_FAILED` and a message naming the endpoint and, for a 401 or 403 alone, the credential. Cancellation during the body read surfaces as `ABORTED`, like a cancellation before the request went out.
...
- **The model catalog and context metadata have different ownership** — `settings.yaml` still decides which model ids a route serves, so metadata refresh never adds or removes models. OpenAI-compatible routes do query `/models` for context capacity on startup/use and after their configured TTL; servers that do not disclose a usable capacity retain the last good value or the configured/catalog fallback.
